...
const systemConfig = {
    diploma: {
        semester: { pre: 'Semesters 1-5 (Single PDF)', final: 'Semester 6', preCode: 'DiplomaSem01to05', finalCode: 'DiplomaSem06' },
        yearly: { pre: 'Year 1-2 (Single PDF)', final: 'Year 3', preCode: 'DiplomaYear01to02', finalCode: 'DiplomaYear03' }
    },
    graduation: {
        semester: { pre: 'Semesters 1-7 (Single PDF)', final: 'Semester 8', preCode: 'GraduationSem01to07', finalCode: 'GraduationSem08' },
        yearly: { pre: 'Year 1-3 (Single PDF)', final: 'Year 4', preCode: 'GraduationYear01to03', finalCode: 'GraduationYear04' }
    },
    pg: {
        semester: { pre: 'Semesters 1-3 (Single PDF)', final: 'Semester 4', preCode: 'Post-GraduationSem01to03', finalCode: 'Post-GraduationSem04' },
        yearly: { pre: 'Year 1 (Single PDF)', final: 'Year 2', preCode: 'Post-GraduationYear01', finalCode: 'Post-GraduationYear02' }
    }
};
...
const fixedFileInputs = {
    ...
    'CV01Page': { id: 'file-CV01Page', pattern: 'B' },
    'CV02Pages': { id: 'file-CV02Pages', pattern: 'B' },
...
const dynamicInputs = {
    ...
    graduation: { preId: 'file-GraduationPreFinal', finalId: 'file-GraduationFinal' },
...
const showStatus = (message, type = 'info') => {
    ...
    statusMessage.className = `mt-4 p-4 rounded-md text-sm font-medium ${type === 'error' ? 'bg-red-50 text-red-700 border border-red-200' :
    ...
    try {
        ...
        for (const [docType, config] of Object.entries(fixedFileInputs)) {
            ...
            if (inputElement && inputElement.files.length > 0) {
                ...
                const newFilename = getRenamedFilename(docType, roll, name, course, branch, config.pattern);
        ...
        for (const [type, inputs] of Object.entries(dynamicInputs)) {
            ...
            if (preInput && preInput.files.length > 0) {
                ...
                const newFilename = getRenamedFilename(docType, roll, name, course, branch, 'A');
        ...
        showStatus(`Successfully downloaded ${filesAdded} document(s) as ZIP!`, "success");
